...
class TeamMemberForm(forms.Form):
    ...
    member_role = forms.ChoiceField(choices=[('player', 'Player'), ('manager', 'Manager'), ('coach', 'Coach')],required=False)
    def clean(self):
...
class ScoreEditForm(forms.Form):
    ...
    def clean(self):
        cleaned_data = super().clean()
        if not (cleaned_data.get('team1_score', -1) >= 0 and cleaned_data.get('team2_score', -1) >= 0):
